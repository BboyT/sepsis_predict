...
chart AS (
SELECT * FROM nursecharting),

apsiii_raw AS (
SELECT * FROM apachepatientresult),

respchart AS (
SELECT * FROM respiratorycharting),
...
where lower(drugname) like '%dopamine%' and infusionoffset between -120 and 1440 and drugrate ~ '^[-]?[0-9]+[.]?[0-9]*$' and drugrate not in ('-','.') 
group by patientunitstayid
...
t3 as  --NOREPI
(
select distinct infusiondrug.patientunitstayid, max(case when lower(drugname) like '%(ml/hr)%' and drugrate<>''  and drugrate<>'.' then round(cast(drugrate as numeric)/300,3) -- rate in ml/h * 16 mcg/ml /admissionweight / 60 min, to convert in mcg/kg/min
when lower(drugname) like '%(mcg/min)%' and drugrate<>'' and drugrate<>'.'  then round(cast(drugrate as numeric)/admissionweight ,3)-- divide by 80 kg
when lower(drugname) like '%(mcg/kg/min)%' and drugrate<>'' and drugrate<>'.' then cast(drugrate as numeric)
else null end ) as norepi


from infusiondrug
left join patient on patient.patientunitstayid = infusiondrug.patientunitstayid
where admissionweight > 0 and 
lower(drugname) like '%norepinephrine%'  and infusionoffset between -120 and 1440  and drugrate ~ '^[-]?[0-9]+[.]?[0-9]*$' and drugrate not in ('-','.') 
group by infusiondrug.patientunitstayid
order by infusiondrug.patientunitstayid
...
from infusiondrug id
where lower(drugname) like '%dobutamin%' and drugrate <>'' and drugrate not in ('-','.') and drugrate ~ '^[-]?[0-9]+[.]?[0-9]*$' and infusionoffset between -120 and 1440
order by patientunitstayid
),
...
from respchart
where respchartoffset between -120 and 1440 and respchartvalue <> '' and respchartvalue ~ '^[-]?[0-9]+[.]?[0-9]*$'
group by patientunitstayid
) as tempo
...
(
select distinct patientunitstayid, min(o2_flow)
    as ncfio2
from pivoted_o2 nc
where chartoffset between -120 and 1440
group by patientunitstayid
...
(
select patientunitstayid,
max(case when lower(labname) like 'pao2%' then labresult else null end) as pao2
from lab
